...
import { onMessage, onMessageFiltrado } from "./handlers/onMessage";
import { getChatroomId, runtimeChannelData, names } from "./utils";

import { initDiscordBot } from './handlers/discordBot';
...
const createNewSocket = async (channels: number[]) => {
  ...
  socket.on("message", (data: WebSocket.Data) => {
    const messageEvent = data.toString();
    onMessageFiltrado(messageEvent);
  });
  socket.on("close", () => {
